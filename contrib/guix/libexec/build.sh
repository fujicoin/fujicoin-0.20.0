...
# Setup the directory where our Fujicoin Core build for HOST will occur
...
if [ -z "$(find "${OUTDIR}/src" -name 'fujicoin-*.tar.gz')" ]; then
    ./autogen.sh
    env CONFIG_SITE="${BASEPREFIX}/${HOST}/share/config.site" ./configure --prefix=/
    make dist GZIP_ENV='-9n' ${V:+V=1}
    mkdir -p "${OUTDIR}/src"
    mv "$(find "${PWD}" -name 'fujicoin-*.tar.gz')" "${OUTDIR}/src/"
fi

# Determine the full path to our source tarball
SOURCEDIST="$(find "${OUTDIR}/src" -name 'fujicoin-*.tar.gz')"
# Determine our distribution name (e.g. fujicoin-0.18.0)
